...
# Função auxiliar para pegar N imagens de uma classe específica
def get_images_of_class(dataloader, class_idx, count=5):
    found_images = []
    found_labels = []
    ...
    for images, labels in dataloader:
        for i in range(len(labels)):
            if labels[i].item() == class_idx:
                found_images.append(images[i].unsqueeze(0)) # [1, 3, 64, 64]
